...
int handle_http_request(const int fd) {
    const int request_buffer_size = 65536; // 64K
    char request[request_buffer_size];
    const int bytes_recvd = recv(fd, request, request_buffer_size - 1, 0);
    ...
    if (bytes_recvd > 0) {
        ...
        const char *p = find_start_of_body(request);
        if (p == NULL) {
        ...
        if (strcmp(request_type, "GET") == 0) {
            ...
            send_response(fd, "HTTP/1.1 200 OK", "None", "", 15);
            close(fd);
            return 0;
        } else {
